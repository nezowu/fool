...
while true; do  #главный цикл в котором все и происходит игровой процесс
	...
	#echo ${string:24:3}
	#if [[ ${battlestaf[]} 
	#echo -e "$X $Y" >>mouse.txt #здесь мы записывали координаты на стадии отладки
	#выходим из игры по нажатии СКМ
	[[ $CLICK == 21 ]] && break
	if [[ $CLICK == 22 && $Y == 43 ]]; then #назначаем действиям для мыши действия
		if [[ $(($X%2)) == 0 ]]; then
			ZNAK=$((($X-33)/2))
			echo $ZNAK
		else
...
#	printHead #обновляем экран и возвращаемся к следующему циклу
fi
done
echo -en "\e[?9l"
